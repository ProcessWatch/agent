...
Once enabled, the agent sends a heartbeat every `reportIntervalSecs` with host metrics, watched process states, and any lifecycle events that occurred since the last one. Reporting runs on its own schedule, separate from local polling, so an unreachable dashboard never delays process checks or recovery. Notifications are configured in the dashboard, not in the local agent.
...
Recovery commands run on Windows — they are executed through `cmd /c`, and the PowerShell examples above work. Two things are weaker than on Linux:

- **No service-state awareness.** ProcessWatch watches Windows targets by process name only. It cannot ask the Service Control Manager whether a service is running, stopped, or failed, so a registered service with no live process is indistinguishable from any other stopped process. Linux is gaining the ability to watch systemd units directly; the Windows equivalent is on the roadmap.
- **No process detachment.** On Linux the agent starts recovery commands in their own process group so they survive independently. On Windows that is a no-op (`restart_windows.go`), so a recovery command that stays in the foreground can be terminated when its timeout expires. Prefer commands that return immediately or detach themselves, such as `cmd /c start "" "C:\path\to\app.exe"`.

Monitoring, reporting, incidents, and notifications behave the same on both platforms. The gap is specific to service-manager integration.
...
|--------|-------------|---------|
| `pollIntervalSecs` | How often the agent checks watched processes locally | `5` |
| `reportIntervalSecs` | How often a heartbeat is sent to the dashboard. Must be >= `pollIntervalSecs`, and determines how long a host may go quiet before the dashboard marks it offline | `15` |
| `restartVerifyDelaySecs` | Delay after a recovery command before verifying the process is healthy | `3` |
| `restartTimeoutSecs` | Maximum time a recovery command may run before the agent kills it | `60` |
...
A lower `pollIntervalSecs` detects failures faster at the cost of local CPU; a lower `reportIntervalSecs` surfaces them on the dashboard sooner at the cost of network traffic and stored rows. The dashboard ingest API is rate-limited, so avoid setting `reportIntervalSecs` below the documented minimum.
...
- Auto-restart toggle
- Max retries and cooldown period (the cooldown spaces out restart attempts, including failed ones)
...
- Windows Service Control Manager support — watch and report service state directly, matching the systemd unit support planned for Linux
- Process detachment for recovery commands on Windows
- Config hot-reload
